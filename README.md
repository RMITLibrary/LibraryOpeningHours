...
Library staff maintain opening hours by editing `hours.txt` — directly, or via the [`hours-editor.html`](docs/hours-editor.html) tool, which loads/edits/downloads a `hours.txt` file entirely in-browser with a live calendar preview and needs no backend. The updated `hours.txt` is then handed to the Library team, who publish it into the AEM-hosted copy (`/2026/hours.txt`). There is no database and no publishing pipeline to operate — updating hours is a matter of replacing one static file.
